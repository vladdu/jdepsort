...
public class DepSortAction extends SelectionDispatchAction {
	...
	/**
	 * Creates a new <code>SortMembersAction</code>. The action requires that
	 ...
	 */
	public DepSortAction(IWorkbenchSite site) {
		super(site);
		setText("Sort methods topologically");
		setDescription("Sort methods topologically");
		setToolTipText("Sort methods topologically");

		// PlatformUI.getWorkbench().getHelpSystem().setHelp(this,
		// IJavaHelpContextIds.SORT_MEMBERS_ACTION);
	}
	 ...
	 *              clients.
	 */
	public DepSortAction(CompilationUnitEditor editor) {
	...
	private boolean hasMembersToSort(IJavaElement[] members)
	...
	private ICompilationUnit getSelectedCompilationUnit(
		...
		if (selection.size() == 1) {
			...
			if (element instanceof ICompilationUnit) {
				...
				if (type.getParent() instanceof ICompilationUnit) { // only top
																	// level
																	// types
